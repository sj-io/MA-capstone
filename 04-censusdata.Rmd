...
The following table uses data from the 5-year American Community Survey to measure changes in population and housing from 2010 to 2019.
...
hsgpopMem <- hsgtenunitMem %>% 
  ...
  mutate(variable = "Total Population in Occupied Housing Units") 
```
...
hsgoccMem <- occstatMem %>% 
  ...
  mutate(variable = "Total Occupied Housing Units") %>% 
  select(-occupancy)
hsgvacMem <- occstatMem %>% 
  filter(occupancy == "Vacant") %>% 
  mutate(variable = "Total Vacant Housing Units") %>% 
  select(-occupancy)

totMem <- rbind(popMem, tothsgMem, hsgoccMem, hsgvacMem, hsgpopMem, gqpopMem) %>% 
...
totmemtbl2 <- totMemtbl %>% 
  pivot_wider(names_from = variable, 
              values_from = c(estimate, change), 
...
kable(chg,
      col.names = c("Variable", "Change from 2010-2019"),
      caption = "Changes in population and housing data, Memphis")
```

From 2010 to 2019, Memphis's population decreased by about 2,944 people. Most of this was a decrease in the number of people living in group quarters[^censusdata-1] (2,589); the population living in occupied units only decreased by 355 individuals over this period. Overall, the population is stagnant.
