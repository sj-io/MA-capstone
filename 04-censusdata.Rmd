...
```{r popP, include=FALSE, cache=TRUE}
popP <- map_dfr(
  yrs,
  ~ get_acs(
    geography = "place",
...
```{r occstatP, include=FALSE, cache=TRUE}
occStatP <- map_dfr(
  yrs,
  ~ get_acs(
    geography = "place",
...
According to the 2015-2019 American Community Survey, there are 251,732 occupied housing units in the city of Memphis.

### Total Population in Occupied Units

<!--# TOTAL POPULATION IN OCCUPIED HOUSING UNITS BY TENURE BY UNITS IN STRUCTURE data -->

```{r HsgTenUnitPlace, include=FALSE, cache=TRUE}
...
hsgoccMem <- occstatMem %>% 
  ...
  select(-occupancy)
hsgpopMem <- hsgtenunitmemYr %>% 
  mutate(variable = "Total Population in Occupied Housing Units") 

totMem <- rbind(popMem, hsgMem, occMem, hsgtenunitmemYr)
```
