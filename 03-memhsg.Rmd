...
```{r medyrbltmem-ten, include=FALSE}
yrbltmem5 <- yrblt5 %>% 
...
```{r medyrbltmsa-ten, include=FALSE}
yrbltmemmsa <- yrbltmsa %>% 
...
knitr::kable(
  list(ownyr5, rntyr5), 
  caption = 'Median year built, occupied housing units in City of Memphis'
  )
```
...
knitr::kable(
  list(ownyrmsa, rntyrmsa), 
  caption = 'Median year built, occupied housing units in Memphis MSA'
  )
```
...
<!--# get PLACE data for TENURE BY YEAR STRUCTURE BUILT -->

```{r yrbltten, include=FALSE}
yrbltten <- get_acs(
  geography = "place",
...
<!--# filter for MEMPHIS, clean up table -->

```{r yrblttenmem, include=FALSE}
yrblttenmem <- yrbltten %>% 
...
yrblttenmem2 <- yrblttenmem %>% mutate(datayr = 2019)

yrbltdecade <- rbind(newyr09, yrblttenmem2)
...
The graph below shows how the number of housing units has changed from the 2009 to the 2019 ACS, based on the year the structure was built, separated by tenure.
...
From 2009 to 2019 there was an overall loss in occupied housing units built before 1939 and between 1960-1969. In every other decade, there was a loss of owner-occupied units and a gain in renter-occupied units. As we cannot travel back in time and build more rental units between 1950-1959, we can assume that these were formerly owner-occupied units that have been converted to rental units.

```{r change0919, eval=FALSE, include=FALSE}
uhh %>% group_by(year) %>% summarise(estimate = sum(estimate))
#' net change from 2009 to 2019 ACS for number of housing units built in each decade
#' notice the increase in 1990-1999. 
#' possible reasons:
#'   1. we did invent time travel and created more units then
#'   2. formerly vacant units built in this period became occupied
#'     - not likely because overall number of housing units decreased 
#'     and vacancies increased over same time period
#'     - loss of 8365 occupied housing units (260097-251732=8365), 
#'     gained 356 vacancies (47825-48181=-356)
#'   3. the range falls in the margin of error (it does)
#'     - 09 MOE: +/- 1474; 19 MOE: +/- 1844 = 3318 variance...
```
...
[^memhsg-2]: In the discussions below, "renters" refers to renter-occupied units, "owners" refers to owner-occupied units, and "Memphis" refers to the Memphis MSA.
